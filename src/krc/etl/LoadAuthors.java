...
public class LoadAuthors {
	...
	private static void saveAuthors(ArrayList<Author> authors, String firstLetter) {
		...
		try {
			...
			//PreparedStatement psdel = conn.prepareStatement("delete from authors where personid=?");
			PreparedStatement psins = conn.prepareStatement("insert into authors (personid, name, surname, lastname, altFIO) values (?, ?, ?, ?, ?)");
			for (Author a : authors) {
				//psdel.clearParameters();
				//psdel.setString(1, a.personid);
				//del = psdel.executeUpdate();
				psins.clearParameters();
				psins.setString(1, a.personid);
				psins.setString(2, a.name);
				psins.setString(3, a.surname);
				psins.setString(4, a.lastname);
				psins.setString(5, a.altFIO);
				psins.execute();
				ins++;
			}
			st.close();
		...
		System.out.println("Удалено/добавлено авторов на '" + firstLetter + "' - " + del + "/" + ins);
	}
